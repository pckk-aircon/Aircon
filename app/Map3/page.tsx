...
const BabylonMap = () => {
  ...
  useEffect(() => {
    ...
    //const worldOrigin = [140.3032013, 35.3537465 - 0.00037];
    const worldOrigin: [number, number] = [140.3032013, 35.3537465 - 0.00037];

    const worldAltitude = 0;

    const worldOriginMercator = maplibregl.MercatorCoordinate.fromLngLat(worldOrigin, worldAltitude);
    const worldScale = worldOriginMercator.meterInMercatorCoordinateUnits() * scale;
    ...
    class BabylonCustomLayer implements maplibregl.CustomLayerInterface {
      id = '3d-model';
      type: 'custom' = 'custom';
      renderingMode: '3d' = '3d';
    ...
    map.on('style.load', () => {
      const layer = new BabylonCustomLayer();
      map.addLayer(layer);
    });
