...
type IotRow = {
  ...
  Power?: string | null;
  WeightedTemp?: string | null;
  ReferenceTemp?: string | null;
  TargetTemp?: string | null;
  PresetTemp?: string | null;
  PanelSetTemp?: string | null;
  SetTemp?: string | null;
  SetTime?: string | null;
  ActualTemp?: string | null;
  ActualHumidity?: string | null;
  ApparentPower?: string | null;
  ActivePower?: string | null;
  CumulativeEnergy?: string | null;
...
export default function App() {
  ...
  const [divisionLists, setPosts] = useState<DivisionRow[]>([]);
  const [deviceLists, setDevices] = useState<DeviceRow[]>([]);
  const [FiltereddeviceLists, setFiltereddevice] = useState<DeviceRow[]>([]);

  const [firstCumulativeEnergy, setFirstCumulativeEnergy] = useState<number | null>(null);

  console.log("FiltereddeviceLists（State直後）=", FiltereddeviceLists);

  // StartDateが変わったら endDate を翌日にする
  useEffect(() => {
    const nextDay = new Date(startDate);
    nextDay.setDate(startDate.getDate() + 1);
  ...
  // Division切替時に、そのDivisionに属する Aircon / Power デバイス一覧を作る
  useEffect(() => {
    if (divisionLists.length > 0 && deviceLists.length > 0) {
      const selectedDivision = divisionLists[currentDivisionIndex]?.Division;
      const filtered = deviceLists.filter(
        (item) =>
      ...
      // Device index が範囲外なら戻す
      if (filtered.length === 0) {
        setCurrentDeviceIndex(0);
      } else if (currentDeviceIndex >= filtered.length) {
        setCurrentDeviceIndex(0);
      }
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [controller, startDate, endDate, currentDivisionIndex, currentDeviceIndex]);

  /**
   * listIot を nextToken で最後までたどって、指定期間の全データを返す
   */
  async function fetchAllIotByRange(startDatetime: string, endDatetime: string): Promise<IotRow[]> {
    const result: IotRow[] = [];
    ...
    do {
      ...
      page += 1;
      console.log(
        `★★★ listIot page=${page} items=${items.length} nextToken=${nextToken ? "あり" : "なし"}`
      );
  ...
  async function listIot() {
    ...
    // 1) IoTデータを全ページ取得
    const allIotData = await fetchAllIotByRange(startDatetime, endDatetime);
    console.log("★★★Iotdata（全件取得後）=", allIotData);

    // 2) Division / Device マスタ取得
    const [
      { data: fetchedDivisionLists, errors: divisionErrors },
      { data: fetchedDeviceLists, errors: deviceErrors },
    ] = await Promise.all([
      client.queries.listDivision({
        Controller: controller,
      }),
      client.queries.listDevice({
        Controller: controller,
      }),
    ]);

    if (divisionErrors?.length) {
      console.error("divisionErrors=", divisionErrors);
    }
    if (deviceErrors?.length) {
      console.error("deviceErrors=", deviceErrors);
    }

    const divisionArray = (fetchedDivisionLists || []) as DivisionRow[];
    const deviceArray = (fetchedDeviceLists || []) as DeviceRow[];

    setPosts(divisionArray);
    setDevices(deviceArray);

    console.log("★currentDivisionIndex（listIot）=", currentDivisionIndex);
    console.log("★currentDeviceIndex（listIot）=", currentDeviceIndex);

    const selectedDivision = divisionArray[currentDivisionIndex]?.Division ?? "";
    const selectedDivisionName = divisionArray[currentDivisionIndex]?.DivisionName ?? "";

    const filteredDeviceArray = deviceArray.filter(
      (item) =>
        item.Division === selectedDivision &&
        (item.DeviceType === "Aircon" || item.DeviceType === "Power")
    );

    setFiltereddevice(filteredDeviceArray);

    const selectedDevice =
      filteredDeviceArray.length > 0
        ? filteredDeviceArray[Math.min(currentDeviceIndex, filteredDeviceArray.length - 1)]?.Device
        : undefined;

    console.log("★selectedDivision（listIot）=", selectedDivision);
    console.log("★selectedDevice（listIot）=", selectedDevice);

    if (allIotData.length > 0) {
      const powerData = allIotData.filter((item) => item?.DeviceType === "Power");
      console.log("☆☆☆powerData=", powerData);

      const firstPowerRow = allIotData.find((item) => item?.DeviceType === "Power");

      let firstCumulativeEnergyValue: number | null = null;
      if (firstPowerRow?.CumulativeEnergy != null) {
        firstCumulativeEnergyValue = parseFloat(firstPowerRow.CumulativeEnergy);
        setFirstCumulativeEnergy(firstCumulativeEnergyValue);
        console.log("☆☆☆firstCumulativeEnergy:", firstPowerRow.CumulativeEnergy);
      } else {
        setFirstCumulativeEnergy(null);
        console.log("Powerデータが見つからないか、CumulativeEnergyが未定義です");
      }

      const formattedData: ChartData[] = allIotData
        .filter((item) => {
        ...
        .map((item) => {
          const cumulativeEnergyValue =
            item?.CumulativeEnergy !== undefined && item?.CumulativeEnergy !== null
              ? parseFloat(item.CumulativeEnergy)
              : null;
          ...
          return {
            ...
            DeviceDatetime: item?.DeviceDatetime ?? "",
            ActualTemp:
              item?.ActualTemp !== undefined && item.ActualTemp !== null
                ? parseFloat(item.ActualTemp)
                : null,
            ActivePower:
              item?.ActivePower !== undefined && item.ActivePower !== null
                ? parseFloat(item.ActivePower)
                : null,
            ApparentPower:
              item?.ApparentPower !== undefined && item.ApparentPower !== null
                ? parseFloat(item.ApparentPower)
                : null,
            CumulativeEnergy: cumulativeEnergyValue,
            InitializedCumulativeEnergy:
              firstCumulativeEnergyValue != null && cumulativeEnergyValue != null
                ? cumulativeEnergyValue - firstCumulativeEnergyValue
                : null,
            WeightedTemp:
              item?.WeightedTemp !== undefined && item.WeightedTemp !== null
                ? parseFloat(item.WeightedTemp)
                : null,
            TargetTemp:
              item?.TargetTemp !== undefined && item.TargetTemp !== null
                ? parseFloat(item.TargetTemp)
                : null,
            PresetTemp:
              item?.PresetTemp !== undefined && item.PresetTemp !== null
                ? parseFloat(item.PresetTemp)
                : null,
            PanelSetTemp:
              item?.PanelSetTemp !== undefined && item.PanelSetTemp !== null
                ? parseFloat(item.PanelSetTemp)
                : null,
            SetTemp:
              item?.SetTemp !== undefined && item.SetTemp !== null
                ? parseFloat(item.SetTemp)
                : null,
            SetTime:
              item?.SetTime !== undefined && item.SetTime !== null
                ? parseFloat(item.SetTime)
                : null,
            ReferenceTemp:
              item?.ReferenceTemp !== undefined && item.ReferenceTemp !== null
                ? parseFloat(item.ReferenceTemp)
                : null,
            Power: item?.Power ?? null,
            ControlStage: item?.ControlStage ?? null,
            Device: item?.Device ?? "",
      ...
      formattedData.sort(
        (a, b) => parseISO(a.DeviceDatetime).getTime() - parseISO(b.DeviceDatetime).getTime()
      );

      console.log("★★★formattedData件数=", formattedData.length);
      if (formattedData.length > 0) {
        console.log(
          "★★★formattedData max DeviceDatetime=",
          formattedData[formattedData.length - 1].DeviceDatetime
        );
      }
  ...
  const mergedData = chartData.map((item) => {
    const newItem: Record<string, any> = {
      DeviceDatetime: item.DeviceDatetime,
      firstCumulativeEnergyLine: firstCumulativeEnergy,
    };

    Object.keys(groupedData).forEach((device) => {
      const deviceData = groupedData[device].find(
        (d) => d.DeviceDatetime === item.DeviceDatetime
      );
      newItem[`${device}_ActualTemp`] = deviceData ? deviceData.ActualTemp : null;
      newItem[`${device}_PanelSetTemp`] = deviceData ? deviceData.PanelSetTemp : null;
    });
    ...
    newItem.PresetTemp = item.ControlMode === "2" ? item.PresetTemp : null;
    newItem.ReferenceTemp = item.ControlMode === "2" ? item.ReferenceTemp : null;
    newItem.PanelSetTemp =
      item.ControlMode === "1" && item.Power === "on" ? item.PanelSetTemp : null;
    newItem.ControlStage = item.ControlStage;
    newItem.ActivePower = item.ActivePower;
    newItem.ApparentPower = item.ApparentPower;
    newItem.CumulativeEnergy = item.CumulativeEnergy;
  ...
  const handlePrevious = () => {
    setCurrentDivisionIndex(
      (prevIndex) => (prevIndex - 1 + divisionLists.length) % divisionLists.length
    );
    setCurrentDeviceIndex(0);
  };

  const DevicehandleNext = () => {
    if (FiltereddeviceLists.length === 0) return;
    setCurrentDeviceIndex((prevIndex) => (prevIndex + 1) % FiltereddeviceLists.length);
  };
  ...
  const DevicehandlePrevious = () => {
    ...
    setCurrentDeviceIndex(
      (prevIndex) => (prevIndex - 1 + FiltereddeviceLists.length) % FiltereddeviceLists.length
    );
  };
    ...
    <main>
      <div>
        <label>
          ...
          <DatePicker
            selected={startDate}
            onChange={(date: Date | null) => setStartDatetime(date ? date : new Date())}
          />
        </label>
        <label>
          EndDatetime:
          <DatePicker
            selected={endDate}
            onChange={(date: Date | null) => setEndDatetime(date ? date : new Date())}
          />
        </label>
      </div>
      ...
      <div>
        <h1>
          Temperature Data for {divisionLists[currentDivisionIndex]?.DivisionName} _{" "}
          {FiltereddeviceLists[currentDeviceIndex]?.DeviceName}
        </h1>
        ...
        <ResponsiveContainer width="100%" height={400}>
          <LineChart data={mergedData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
            ...
            {Object.keys(groupedData).map((device, index) =>
              deviceNameMapping[device] ? (
                <Line
                  key={`${device}_ActualTemp`}
                  type="monotone"
                  dataKey={`${device}_ActualTemp`}
                  name={`${deviceNameMapping[device]} ActualTemp`}
            ...
            >
              <LabelList
                dataKey="ControlStage"
                position="top"
                style={{ fontSize: "6px", fill: "#000" }}
              />
            </Line>
