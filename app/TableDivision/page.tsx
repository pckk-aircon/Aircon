      ...
      next: (event) => {
        ...
        setPosts((prevPosts) => {
          if (!prevPosts.some((post) => post.Division === event.Division)) {
            return [...prevPosts, event as { Division: string; DivisionName: string; DivisionGeojson: string; Controller?: string | null }];
          }
          return prevPosts;
        });
      },
      ...
      <ul>
        {posts.map((post) => (
          <li key={post.Controller}>
            {post.Division}{post.DivisionName}{post.DivisionGeojson}{post.Controller}
          </li>
        ))}
      </ul>
    </main>
